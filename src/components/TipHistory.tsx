...
export const TipHistory = ({
  ...
  return <Card className="p-6 px-[25px] rounded-sm">
      {!limit && <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-semibold">
              {isDashboard ? qrCodeId ? "QR Code Tips" : "Recent Tips" : `${authorName}'s Activity`}
            </h2>
            {isDashboard && <TipDownloadButton tips={tips || []} likes={likes || []} comments={comments || []} qrCodeId={qrCodeId} />}
          </div>
        </div>}

      <TipTable tips={tips || []} authorId={authorId} likes={likes || []} comments={comments || []} showAll={showAll} setShowAll={setShowAll} onSelectTip={setSelectedTip} limit={limit} />

      <TipDetailsDialog isOpen={!!selectedTip} onClose={() => setSelectedTip(null)} tip={selectedTip} />
    </Card>;
};