...
interface EmailNotificationRequest {
  type: NotificationType;
  userId: string;
  data?: Record<string, any>; // Additional data for each notification type
}
...
serve(async (req) => {
  ...
  try {
    const { type, userId, data = {} } = await req.json() as EmailNotificationRequest;
    ...
    if (!email) {
      ...
      return new Response(
        JSON.stringify({ error: "User email not found in profile" }), 
